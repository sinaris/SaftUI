...
UF.Colors = setmetatable({
	...
	power = setmetatable({
		...
		["POWER_TYPE_FEL_ENERGY"] = {224/255, 224/255, 150/255},
...
function UF:UpdateColors()
	local config = S.Saved.profile.UnitFrames.Colors
	--Loop through each color config option
	for colorKey, colorValues in pairs(config) do
		--Check to make sure this color type actually exists
		if self.Colors[colorKey] then
			self.Colors[colorkey] = config[colorKey]
		end
	end
end