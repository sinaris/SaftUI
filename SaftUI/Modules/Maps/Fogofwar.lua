...
local worldMapCache, battleMapCache, discoveredOverlays = {}, {}, {}
...
function WM:GetNumMapOverlays()
	return 0
end

function WM:RealHasOverlays()
	local mapFile = GetMapInfo()
	if not mapFile or not errata then return false end
	
	local overlayMap = errata[mapFile]
...
function WM:UpdateWorldMapOverlays()
	if not WorldMapFrame:IsShown() then return end
	UpdateOverlayTextures(WorldMapDetailFrame, "WorldMapOverlay%d", worldMapCache, 1, 0)
end

function WM:UpdateBattlefieldMinimapOverlay()
	if not BattlefieldMinimap or not BattlefieldMinimap:IsShown() then return end
	UpdateOverlayTextures(BattlefieldMinimap, "BattlefieldMinimapOverlay%d", battleMapCache, BattlefieldMinimap1:GetWidth() / 256, BattlefieldMinimapOptions.opacity)
end


function WM:LoadFogOfwar()
	-- setup the overlays
	errata["IsleoftheThunderKing"] = false
	errata["IsleoftheThunderKingScenario"] = false
	...
	for i = 1, NUM_WORLDMAP_OVERLAYS do
		tinsert(worldMapCache, _G[("WorldMapOverlay%d"):format(i)])
	end
	...
	if not IsAddOnLoaded("Blizzard_BattlefieldMinimap") then
		self:RegisterEvent("ADDON_LOADED", function(event, addon)
			if addon == "Blizzard_BattlefieldMinimap" then
				WM:SecureHook("BattlefieldMinimapUpdate", "UpdateBattlefieldMinimapOverlays")
				WM:UnregisterEvent("ADDON_LOADED")
			end
		end)
	else
		...
		for i = 1, BattlefieldMinimap:GetAttribute("NUM_BATTLEFIELDMAP_OVERLAYS") do
			tinsert(battleMapCache, _G[("BattlefieldMinimapOverlay%d"):format(i)])
		end
